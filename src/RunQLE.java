...
public class RunQLE {
	...
	public static void main(java.lang.String[] args) {
		...
		long t;
		try {
			PrintWriter summary = new PrintWriter(new FileWriter(new File(homedir, "sim." + K + "." + n + ".qle.summary.txt")));
			PrintWriter out = new PrintWriter(new FileWriter(new File(homedir, "sim." + K + "." + n + ".qle.txt")));
			out.print("size i phi ");
			for (int i = 0; i < K; i++) {
				out.print("Theta" + i + " ");
			}
			out.println("PhiMLE PhiQLE PhiQLEh PhiQLEv");
			for (int k = 0; k < phi.length; k++) {
				...
				for (int j = 0; j < N; j++) {
					...
					T.mCLETrueX();
					meanMLETrueX += T.phi;
					varMLETrueX += T.phi * T.phi;
					out.print(T.phi + " ");
					T.phi = 0;
					T.theta = (double[]) theta.clone();
					t = System.currentTimeMillis();
					T.mQLENearestAverege();
					meanMnA += T.phi;
					varMnA += T.phi * T.phi;
					out.print(T.phi + " " + T.TNearestAvg[0].phi + " " + T.TNearestAvg[1].phi + " "
